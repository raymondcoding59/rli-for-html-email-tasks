...
- Some types of elements/rows should idealy be grouped together in one parent row, these include:

  - A heading with its corresponding subheading or supporting text
  - A product image with its corresponding product name and price text
  - Lists and bullet points, including their intro and outro(if applicable)
  - A group of elements that share the same background color, background image or bounded by a container/border, even if they are not directly adjacent to each other in the design (e.g., a section with a colored background that contains multiple rows of elements, such as headings, text blocks, images, buttons, etc. that are all part of the same section and share the same background color/image). These should be considered as rows of on parent row, and would technically be implemented as a single table row with multiple nested tables for the inner rows and elements. The parent row should be noted as such, and the inner rows should be listed as sub-rows under the parent row, with their corresponding elements listed under each sub-row.
...
The JSON should follow thesame format in specification-guide.json, use placehold.co for URLs of images and Icons(with .png file type)


##

Analyse this email design and determine all the specifications in specification-placeholder.json. Your output should be a JSON file with the filled values
