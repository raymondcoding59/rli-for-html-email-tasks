...
Your task is to generate HTML for a new email design while strictly following the coding style and structure of a provided reference email.

CRITICAL RULE:
The reference HTML must be treated as the BASE TEMPLATE. You are NOT allowed to rebuild the email from scratch. You must MODIFY the reference HTML only where the design requires changes.
...
- inline styles
- spacing utilities
- MJML compiled classes
- klaviyo component wrappers

The following must remain identical unless the design forces a change:
...
5. WHEN REMOVING CONTENT
Remove only the smallest possible block while keeping parent wrappers intact.

6. OUTPUT FORMAT
Return ONLY the final HTML in one fenced code block.
...
- container width remains identical
