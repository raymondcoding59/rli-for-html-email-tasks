...
def edit_chunk_file(chunk_path, layout_item):
    ...
    prompt = f"""
You are editing an email HTML block.

STRICT RULES:
- DO NOT change structure
- DO NOT remove or add wrappers
- KEEP all tables/divs EXACT
...
# -----------------------------
# BUILD EMAIL (NO GLOBAL MAPPING)
# -----------------------------
def build_email(layout, embeddings):
    ...
    for item in layout_items:
        ...
        # prevent over-reuse (optional)
        if chunk_path in used_chunks:
            continue
        ...
        final_chunks.append(edited_chunk)
...
def run_pipeline(reference_path, design_path):
    ...
    # 4. Build email (retrieval-based)
    final_html = build_email(layout, embeddings)
