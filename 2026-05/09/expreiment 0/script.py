...
def find_best_chunks(layout_item, embeddings, k=3):
    ...
    query_response = client.embeddings.create(model="text-embedding-3-small", input=query_text)
    log_tokens(
...
def compact_reference_html(chunk_html, limit=6500):
...
def build_email(reference_html, design_spec, embeddings, image_memory):
    ...
    for index, section in enumerate(sections):
        layout_item = {key: value for key, value in section.items() if key != "content"}
        best_chunks = find_best_chunks(layout_item, embeddings, k=2)
